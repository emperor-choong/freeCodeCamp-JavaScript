function analyzeIdeas(ideas) {
    let array = ideas.map(element => {
        let [ideaName, optimistic, realistic, pessimistic] = element;
        let expectedTime = ((optimistic + 4 * realistic + pessimistic) / 6) * ideaName.length;

        return [ideaName, expectedTime];
    });

    // Array destructuring
    let result = array.sort((a, b) => a[1] - b[1]).map(([key]) => key);

    return result;
}

// console.log(analyzeIdeas([["Add logging", 2, 5, 15], ["SEO optimization", 4, 8, 20], ["Fix bug", 1, 3, 5]]));
// console.log(analyzeIdeas([["Dark mode", 1, 3, 8], ["Real-time collaboration feature", 6, 12, 20], ["Add tooltip", 1, 2, 4]]));
